...
{
    int h, i, j, k, l, wli, iend, generation, newPhotons=T1.size(), nR=16384, nT=1919;
    bool sx,sy,sz;
    ...
    std::vector<double> parent;  parent.resize(newPhotons); //Xa = [x y z nxr nyr nzr gen]
     ...
     for (generation=0; generation<5000; generation++){
         ...
         if(newPhotons<5 && generation>30){break;}
         ...
         for (h=0; h<iend; h++){
             ...
             if (generation==0){
                 ...
                 if (S[i]==4){
                     isovecs(&v2x,&v2y,&v2z); V1[j]=v2x; V1[k]=v2y; V1[l]=v2z;
                     T1[i] = T2[i] + RT[  (int) ((float)rand()/RAND_MAX*16384) ];
                     wl[i] = wl[i] + RWL[ (int) ((float)rand()/RAND_MAX*16384) ];
                 }
                 S[i]=0;
             }
            ...
            if (S[i]==0){
                if (RN < LT[wli+3*nT]){
                    S[i]=4;  parent[newPhotons]=i;  newPhotons++;}
                continue;
            } //S0 ATTENUATED, S4 RE-EMISSION
            
            
            if (RN < .01){
                diffusevecc(nx,ny,nz,&v2x,&v2y,&v2z);  
                //isovecs(&v2x,&v2y,&v2z);
                V1[j]=v2x; V1[k]=v2y; V1[l]=v2z;
                S[i]=5;  parent[newPhotons]=i;  newPhotons++;
                continue;
            ...
            if (RN < R){ //S6 SPECULAR REFLECTION
                v2x = ux + 2*ct*nx;
                v2y = uy + 2*ct*ny;
                v2z = uz + 2*ct*nz;  V1[j]=v2x; V1[k]=v2y; V1[l]=v2z;
                S[i]=6;  parent[newPhotons]=i;  newPhotons++;
                continue;
            }

            S[i] += 10; //TRANSMISSION!  S11=XTRANSMISSION, S12=YTRANSMISSION, S13=ZTRANSMISSION
             
         } // for photon
         iend=newPhotons;
         //parent.resize(newPhotons);
